...
func NewDefaultLogConfig()*conf.ZapLogConfig  {
	return &conf.ZapLogConfig{
		LogLevel:       "info",
		LogFileName:    "logs.log",
		LogSectionSize: 27, //128MB
	}
}
...
func InitLog(config *conf.ZapLogConfig) {
	fileName := config.LogFileName
	level := getLoggerLevel(config.LogLevel)
	syncWriter := zapcore.AddSync(&lumberjack.Logger{
		Filename:  fileName,
		MaxSize:   1 << config.LogSectionSize, //1G
		LocalTime: true,
		Compress:  true,
	})
